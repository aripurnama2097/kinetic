...
<?php
  ...
  // $split_qty   =$param[0]->split_qty;
  // $stdpack         = $param[0]->stdpack;

  // $scan_issue      =$param[0]->scan_issue;
  ...
  foreach ($param as $key => $value) {
      ...
      echo '<td>: ' . substr($value->split_qty, 0, 15) . '</td>';
 ...
 $barcode = $value->partno . ':' . $value->partname . ':' . $value->split_qty . ':' . $value->dest . ':' . $value->custpo . ':' . $value->shelfno . ':' . $value->idnumber;
          ...
          $data .= $esc . 'H0040' . $esc . 'V0235' . $esc . 'L0101' . $esc . 'M' . 'QTY: ' . substr($value->split_qty, 0, 15);
