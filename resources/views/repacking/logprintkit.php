...
// die();
  ...
  foreach ($param as $key => $value) {
      ...
      echo '<td>: ' . substr($value->qty_scan, 0, 15) . '</td>';
 ...
 $barcode = $value->partno . ':' . $value->partname . ':' . $value->qty_scan . ':' . $value->dest . ':' . $value->custpo . ':' . $value->shelfno . ':' . $value->idnumber;
          ...
          $data .= $esc . 'H0040' . $esc . 'V0235' . $esc . 'L0101' . $esc . 'M' . 'QTY: ' . substr($value->qty_scan, 0, 15);
